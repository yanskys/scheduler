...
module.exports = function (agenda, options) {
  ...
  const requeueJobs = async (jobIds) => {
    ...
    for (const job of jobs) {
      ...
      // if (job.type) newJob.type = job.type;
      // if (job.endDate) newJob.endDate = job.endDate;
      // if (job.nextRunAt) newJob.schedule(job.nextRunAt);
      // if (job.repeatInterval) newJob.repeatEvery(job.repeatInterval);
      // console.log(job)
